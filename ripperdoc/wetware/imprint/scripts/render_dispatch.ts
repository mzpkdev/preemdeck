...
/**
 * render_dispatch.ts — JOBS ASCII-tree panel from status flags (port of
...
function makeNode(partial: Partial<Node> & { status: string }): Node {
  return {
 ...
 */
export function scanArg(arg: string): [string[], boolean] {
  const pieces: string[] = [];
  let cur: string[] = [];
  let endsOpen = false;
  let i = 0;
...
export function splitWaves(args: string[]): string[][] {
  const waves: string[][] = [];
...
export function parse(argv: string[]): Node[] {
  const nodes: Node[] = [];
  let i = 0;
  const n = argv.length;
  while (i < n) {
