...
class RenderDispatchTest(unittest.TestCase):
    ...
    def test_interleaved_order_preserved(self) -> None:
        expected = "DISPATCH  ▰▰▱  2/3\n├── ⏹︎ A\n├── ▶︎ B\n└── ⏹︎ C"
        self.assert_panel(["--done", "A", "--running", "B", "--done", "C"], expected)

    # 5 — a single running wave nests its members under a bare `parallel` node
    def test_single_running_wave(self) -> None:
        expected = "DISPATCH  ▱▱▱  0/3\n└── parallel\n    ├── ▶︎ p\n    ├── ▶︎ q\n    └── ▶︎ r"
        self.assert_panel(["--running", "p,q,r"], expected)

    # 6 — a pending wave uses the queued glyph ⏸︎
    def test_pending_wave(self) -> None:
        expected = "DISPATCH  ▱▱  0/2\n└── parallel\n    ├── ⏸︎ lint\n    └── ⏸︎ types"
    ...
    def test_wave_not_last_uses_pipe(self) -> None:
        expected = "DISPATCH  ▰▱▱  1/3\n├── parallel\n│   ├── ▶︎ x\n│   └── ▶︎ y\n└── ⏹︎ z"
        self.assert_panel(["--running", "x,y", "--done", "z"], expected)

    # 9 — blocked job draws ⏏︎ and appends ` — waits on X`
    def test_blocked_waits_on(self) -> None:
        expected = "DISPATCH  ▰▱  1/2\n├── ⏹︎ scout\n└── ⏏︎ verify — waits on parallel"
        self.assert_panel(
