 ...
 *
 * Mirrors the original's monotonic-clock seam: every duration in the core (recv
 ...
 * match the original contract exactly — a float count of seconds that only ever
 * moves forward.
 */
...
/** Default clock — `performance.now()` (ms) in SECONDS, mirroring the original's monotonic clock. */
export const monotonic: Clock = () => performance.now() / 1000

/**
 * A controllable clock that returns its mutable `.now`, mirroring the original's
 * `_FakeClock`. Reads return the current value; tests assign
...
/** Build a `FakeClock` seeded at `start` (default 1000, matching the original seam). */
