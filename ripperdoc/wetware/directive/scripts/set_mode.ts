...
/**
 * set_mode.ts — the deterministic preemdeck.json writer (port of set_mode.py).
 *
 ...
 * via lib/jsonStore.ts (2-space indent + trailing newline, byte-identical). Same
 * input -> same bytes.
 *
...
export function availableModes(skillsDir: string): string[] {
 ...
 */
export function slotFor(modesFile: string, value: string): string | null {
  let data: unknown;
  try {
...
export function configSlots(config: string): string[] {
  let data: unknown;
  try {
...
export async function setDirective(config: string, slot: string, value: string): Promise<void> {
  let data: unknown;
  try {
    data = JSON.parse(readFileSync(config, "utf8"));
  } catch {
