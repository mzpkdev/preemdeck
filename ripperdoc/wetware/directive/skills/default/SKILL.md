...
```bash
"$HOME/.preemdeck/scripts/preemdeck-bun" "${CLAUDE_PLUGIN_ROOT}/scripts/set_mode.ts" <value>
```
...
pass that through, don't retry with a guess. `set_mode.ts` is the only writer of `preemdeck.json`. The directive takes
