...
shell-out to a browser), confirm a live IDE via resolve_exec_path (a JetBrainsError
...
def test_open_url_resolve_guard_runs_before_preview(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    previewed = _stub_seams(monkeypatch, resolve_raises=JetBrainsError("no JetBrains IDE in the process ancestry"))
...
def test_main_no_live_ide_returns_1(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    # No running IDE -> open_url raises JetBrainsError -> exit 1 on stderr. There
