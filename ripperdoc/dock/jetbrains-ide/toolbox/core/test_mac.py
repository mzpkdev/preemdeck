...
def test_in_jetbrains_true_via_bundle_id(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("__CFBundleIdentifier", "com.jetbrains.WebStorm")
    monkeypatch.delenv("TERMINAL_EMULATOR", raising=False)
    assert in_jetbrains() is True


def test_in_jetbrains_true_via_terminal_emulator(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("__CFBundleIdentifier", raising=False)
    monkeypatch.setenv("TERMINAL_EMULATOR", "JetBrains-JediTerm")
    assert in_jetbrains() is True


def test_in_jetbrains_false_when_neither_set(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("__CFBundleIdentifier", raising=False)
    monkeypatch.delenv("TERMINAL_EMULATOR", raising=False)
    assert in_jetbrains() is False
