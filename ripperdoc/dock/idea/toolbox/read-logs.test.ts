...
import { _internals, main, readLogs } from "./read-logs.ts";
...
beforeEach(async () => {
  ...
  _internals.resolveLogDir = async () => dir;
...
describe("readLogs", () => {
  ...
  test("propagates IdeaError from resolveLogDir", async () => {
    _internals.resolveLogDir = async () => {
...
describe("main", () => {
  ...
  test("non-int arg -> exit 2 with usage", async () => {
    const exitSpy = spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${code}`);
    }) as never);
    const errSpy = spyOn(process.stderr, "write").mockImplementation((() => true) as never);
    try {
      await expect(main(["abc"])).rejects.toThrow("exit:2");
