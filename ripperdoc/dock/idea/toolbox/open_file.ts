#!/usr/bin/env bun
/**
 * open_file.ts — open a file in the running JetBrains IDE. Behavior-identical TS
...
function parseIntArg(name: string, raw: string): number {
  if (!/^[+-]?\d+$/.test(raw.trim())) {
 ...
 */
export async function openFile(path: string, options: OpenFileOptions = {}): Promise<string | null> {
