...
/** CLI entrypoint: parse argv argparse-faithfully, then exit 0 inside a JetBrains IDE / 1 outside (the shell gate). */
export const main = (argv: string[] = Bun.argv.slice(2)): number => {
  ...
  try {
    ...
    if (exc instanceof NotImplementedError) {
      process.stderr.write(`in-idea: ${exc.message}\n`);
      return 1;
    }
    throw exc;
  }
