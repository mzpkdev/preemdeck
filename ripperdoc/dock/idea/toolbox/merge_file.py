...
from core import IdeaError, in_idea, launch, reap_later
...
def merge_file(target: str, suggestion: str, base: str | None = None, *, wait: bool = False) -> str | None:
    ...
    `merge <local> <remote> [<base>] <output>`. The inputs are never touched; the
    ...
    called with the default `wait=False` (async spawn, no `--wait`) and joins the
    process itself with `proc.wait()`.
    ...
    (reap_later) rather than leaked, since there is no synchronous signal for when
    the IDE is done with it. With `wait=True`, blocks on the spawned process,
    then reads back and returns the resolved output's full text, unlinking the
    output temp on the way out (try/finally — wait path only, after the read).

    `launch()` is the single guard for a live IDE: it raises IdeaError if none
    is found.
    """
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  merge_file.py mine.py theirs.py base.py   # 3-way merge with a base\n"
            "  merge_file.py mine.py theirs.py --wait    # block until applied, then print"
        ),
    ...
    try:
        ...
        # launch()'s deeper resolve_exec_path() ancestry walk. Reuse the
