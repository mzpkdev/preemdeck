...
An IdeaError out of open_file surfaces as exit 1.
"""
...
def _in_idea(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(open_inline, "in_idea", lambda: True)
...
def test_open_inline_idea_error_propagates(monkeypatch: pytest.MonkeyPatch) -> None:
    # open_file is the live-IDE guard: its IdeaError propagates out.
    seen_paths: list[str] = []
...
def test_main_no_live_ide_returns_1(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    # No running IDE -> open_inline raises IdeaError -> exit 1 on stderr.
...
def test_main_outside_jetbrains_returns_1_before_work(
    ...
    # exits 1 with the canonical resolver message and never reaches open_inline().
    monkeypatch.setattr(open_inline, "in_idea", lambda: False)
    captured = _capture_open_inline(monkeypatch)
    assert open_inline.main(["body"]) == 1
    assert captured == []
    assert "open_inline: no JetBrains IDE in the process ancestry" in capsys.readouterr().err
