/**
 * idea_mac.test.ts — hermetic, no real `ps` or IDE. Port of test_mac.py with the
...
import { inIdea, type PsProbe, resolveExecPath, resolveLogDir } from "./idea_mac.ts";
...
function fakeProbe(ancestry: Record<number, [number, string]>): PsProbe {
  return (pid) => {
