/**
 * _groovy.test.ts — the shared ideScript bridge. Ports the escape rules + the
...
function launchSpy(raises?: unknown) {
...
function deps(spy: ReturnType<typeof launchSpy>): { deps: RunGroovyDeps; reaped: string[][]; warned: string[] } {
  const reaped: string[][] = [];
  const warned: string[] = [];
  return {
    reaped,
