"""JetBrains IDE detection for the idea toolbox (macOS).
...
def resolve_exec_path() -> str:
    ...
    chain to it. Raises IdeaError if no JetBrains IDE is in the ancestry.
...
def resolve_log_dir() -> Path:
    ...
    if not dirs:
        raise IdeaError(f"no log dir for {product!r}")
    return dirs[0]
