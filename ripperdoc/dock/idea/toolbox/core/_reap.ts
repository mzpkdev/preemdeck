/**
 * _reap.ts — deferred temp-file cleanup for the toolbox's fire-and-forget
 ...
 */
export function reapLater(paths: Iterable<string>, delayMs: number = REAP_DELAY_MS): void {
  const targets = [...paths];
  setTimeout(() => {
    void reap(targets);
  }, delayMs);
...
async function reap(targets: string[]): Promise<void> {
