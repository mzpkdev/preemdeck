/**
 * os_ding.test.ts — port of test_os_ding.py. Hermetic: no real audio. The run
...
import { ding, dingLinux, dingMacos, LINUX_CANDIDATES, platformWorker, runCmd } from "./os_ding.ts";

// A fake run that records argv and answers per a predicate.
function fakeRun(ok: (cmd: string[]) => boolean): { calls: string[][]; run: (cmd: string[]) => Promise<boolean> } {
  const calls: string[][] = [];
  return {
    calls,
    run: (cmd: string[]) => {
