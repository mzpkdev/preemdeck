...
describe("ENV.PLUGIN_ROOT / ENV.MARKETPLACE_ROOT", () => {
    ...
    context("when invoked from a plugin script under ripperdoc", () => {
        beforeEach(() => {
            process.argv[1] = "/home/u/.preemdeck/source/ripperdoc/wetware/ghost/scripts/boot.ts"
        })

        it("resolves MARKETPLACE_ROOT to the rack directly under ripperdoc", () => {
            expect(ENV.MARKETPLACE_ROOT).toBe("/home/u/.preemdeck/source/ripperdoc/wetware")
        })

        it("resolves PLUGIN_ROOT to the plugin beneath the rack", () => {
            expect(ENV.PLUGIN_ROOT).toBe("/home/u/.preemdeck/source/ripperdoc/wetware/ghost")
        })
    })

    context("when invoked from a deeper toolbox script", () => {
        it("anchors on ripperdoc regardless of how deep the entry script sits", () => {
            process.argv[1] = "/x/.preemdeck/source/ripperdoc/dock/idea/toolbox/core/reap.ts"
            expect(ENV.MARKETPLACE_ROOT).toBe("/x/.preemdeck/source/ripperdoc/dock")
            expect(ENV.PLUGIN_ROOT).toBe("/x/.preemdeck/source/ripperdoc/dock/idea")
        })
    })
