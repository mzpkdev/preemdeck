...
- The `wire:start`/`wire:stop` runtime path runs the TS app via
  `"$HOME/.preemdeck/preemdeck-runtime" "${CLAUDE_PLUGIN_ROOT}/apps/wire/start.ts"` (and `stop.ts`).
...
overlay (`src/overwrite/<harness>/` — settings + the worker agents `fixer` / `runner` / `critic` / `scanner`) into the
host config dir, backing up any clobbered file once to `<file>.bak`. So editing this repo does **not** update a running
harness: overlay edits need a re-install to copy out again, and any edit only takes effect after the host CLI restarts.
