...
The bootstrap clones the source from `main` to `~/.preemdeck` (its own directory — your `~/.claude` / `~/.codex` /
`~/.gemini` config is left in place) and runs `install.ts <harness>`. The default harness is `claude`; pass another to
target it — `… | bash -s codex`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/mzpkdev/preemdeck/main/boot.sh | bash    # re-run to refresh ~/.preemdeck from main + re-install
