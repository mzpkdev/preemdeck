...
/**
 * scripts/format_on_edit.ts — format the file an agent just edited.
 *
...
async function readPayload(stdin: { text(): Promise<string> }): Promise<Record<string, unknown> | null> {
  let parsed: unknown;
  try {
...
async function format(path: string): Promise<void> {
...
export async function main(stdin: { text(): Promise<string> } = Bun.stdin): Promise<void> {
...
// Exported for tests; the suffix→formatter map + helpers are unit-tested directly.
