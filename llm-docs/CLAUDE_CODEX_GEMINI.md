...
Cross-host cheat sheet for plugin/skill authoring. Verified against Claude Code, Codex, and Gemini CLI docs, 2026-06-30.

> **Heads-up (Gemini):** Google is transitioning Gemini CLI to **Antigravity CLI** (announced ~2026-05; free /
> Google-One tiers stopped being served by Gemini CLI on 2026-06-18). Skills, hooks, subagents, and extensions carry
> forward as "Antigravity plugins"; the Gemini CLI repo/docs remain authoritative for now — treat the Gemini column as
> current but renamed-in-progress.
...
marketplace catalog   (at marketplace root — Gemini has no equivalent)
  ...
  Gemini  → —   (no catalog file; install one at a time: `gemini extensions install <source>` — GitHub URL or local path. Discovery-only gallery at geminicli.com/extensions)
...
| Marketplace catalog          | `.claude-plugin/marketplace.json` (schema: `name`/`owner`/`plugins[]`; `description` optional)                                                                                                                                                                                                                                                            | `.agents/plugins/marketplace.json` native; also reads `.claude-plugin/marketplace.json` as compat (schema: `name`/`interface.displayName`/`plugins[].source`/`policy`/`category`)                                                                                                                         | No catalog file — install one at a time: `gemini extensions install <source>` (GitHub URL or local path); discovery-only gallery at geminicli.com/extensions                   |
...
| Multi-agent comms            | `Agent` (pass `name:` to spawn a teammate) + `SendMessage` — one implicit team per session; `TeamCreate`/`TeamDelete` removed v2.1.178 (`team_name` still accepted, ignored)                                                                                                                                                                              | Built-in subagents (`default`/`worker`/`explorer`) + user/project `.toml` files; batch fan-out via `spawn_agents_on_csv` + `report_agent_job_result`; `/agent` switches threads; enabled by default                                                                                                       | Subagents exposed as tools to the main agent only — no peer-to-peer                                                                                                            |
...
| Built-in subagent catalog    | (none — define your own)                                                                                                                                                                                                                                                                                                                                  | `default`, `worker`, `explorer`                                                                                                                                                                                                                                                                           | `generalist`, `cli_help`, `codebase_investigator`, `browser_agent` (browser_agent disabled by default, added v0.37.0)                                                          |
| Ask-user tool (LLM-callable) | `AskUserQuestion` — 1-4 Q per call, 2-4 opts each, `multiSelect`, `preview` (markdown box); 60s timeout                                                                                                                                                                                                                                                   | `ask_user_question` — single/multi-choice options with optional custom-answer slot per question                                                                                                                                                                                                           | `ask_user` (v0.29.0+) — 1-4 Q; types `choice` / `text` / `yesno`; `header` ≤16ch; `multiSelect` auto-adds "All the above"                                                      |
| Plan mode                    | **LLM-callable**: `EnterPlanMode` / `ExitPlanMode` tools — the agent can enter and exit programmatically                                                                                                                                                                                                                                                  | **User-toggled only** — no LLM-callable plan-mode tool. Mode set via host UI (`/plan` slash command or `Shift+Tab`); the LLM operates within whatever mode the user picked                                                                                                                                | **LLM-callable** — `enter_plan_mode` / `exit_plan_mode` tools (also `/plan`, Shift+Tab). Read-only filesystem; markdown writes restricted to the plans directory               |
| Task tracking (LLM-callable) | `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet` / `TaskStop` (+ `TaskOutput`, deprecated) — statuses `pending` / `in_progress` / `completed`; `TodoWrite` off by default since v2.1.142                                                                                                                                                              | `update_plan` — array of `{step, status}`; statuses `pending` / `in_progress` / `done`                                                                                                                                                                                                                    | `write_todos` — array of `{description, status}`; statuses `pending` / `in_progress` / `completed` / `cancelled` / `blocked`                                                   |
| Cross-session memory tool    | —                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                                                                                                                                                                                                                         | Tiered Markdown via `write_file`/`replace` into `GEMINI.md` (`save_memory` tool removed v0.40.0)                                                                               |
...
| Effort levels                | `low` / `medium` / `high` / `xhigh` / `max` (default `high` on Opus 4.8 — was `xhigh` on Opus 4.7; `xhigh` needs Opus 4.7+, older models fall back to `high`; `max` works back to Opus 4.6 / Sonnet 4.6. `effortLevel` setting takes `low`–`xhigh`; `max` is session-only unless pinned via `CLAUDE_CODE_EFFORT_LEVEL`. Levels recalibrated for Opus 4.8) | —                                                                                                                                                                                                                                                                                                         | —                                                                                                                                                                              |
...
| Tool restriction unit        | Per-skill (`allowed-tools`)                                                                                                                                                                                                                                                                                                                               | Per-agent TOML (`sandbox_mode`, `mcp_servers`; no `tools` allowlist field)                                                                                                                                                                                                                                | Per-extension (`excludeTools`) + per-agent (frontmatter `tools` allowlist)                                                                                                     |
| Per-subagent policy gate     | Permissions are session-wide — see `claude --agent <name>` for whole-session scoping                                                                                                                                                                                                                                                                      | Per-agent `.toml` only — no main-side enforcement surface                                                                                                                                                                                                                                                 | any `*.toml` in `~/.gemini/policies/` — `[[rule]] subagent = "name"` (decisions `allow`/`deny`/`ask_user`); the only documented surface that sees subagent identity at runtime |
...
| Assistant message shown   | `MessageDisplay`      | —                   | —                     |
| Before context compaction | `PreCompact`          | `PreCompact`        | `PreCompress`         |
| After context compaction  | `PostCompact`         | `PostCompact`       | —                     |
...
| Subagent finishes         | `SubagentStop`        | `SubagentStop`      | —                     |
...
| `decision` + `reason`          | `"block"`                                | `"block"`                                                                                              | `"allow"` / `"deny"` (`"block"` = alias of `deny`)                      |
...
| `updatedInput` (PreToolUse)    | inside `hookSpecificOutput.updatedInput` | inside `hookSpecificOutput.updatedInput`                                                               | `hookSpecificOutput.tool_input` (override field; no `updatedInput` key) |
| `permissionDecision`           | `allow`/`deny`/`ask`/`defer`             | PreToolUse: `allow`/`deny` (`ask` parsed, unsupported); PermissionRequest → `decision.behavior` object | —                                                                       |
...
hook process, so `${GEMINI_PROJECT_DIR}` only resolves via shell expansion inside hook `command` strings — it isn't
...
| Codex `tool_name` is `apply_patch` for file edits                          | Matcher accepts `Edit` / `Write` as aliases, but for file edits the stdin payload's `tool_name` is _always_ `apply_patch` (Bash and MCP calls report their canonical names) — a script that branches on `tool_name == "Edit"` never matches                                                                                                                                                                                                                                                                                                      | Branch on `apply_patch`, or look at `tool_input.file_path` instead                                                                                                                                                                                                                                                  |
...
| MCP-tool hook matching on Codex                                            | Now supported (match `mcp__server__tool` / `mcp__server__.*`); older versions matched only Bash + `apply_patch`                                                                                                                                                                                                                                                                                                                                                                                                                                  | Pin a minimum Codex version                                                                                                                                                                                                                                                                                         |
...
Multi-agent   ── Claude has Agent(name:)/SendMessage (implicit team; TeamCreate/TeamDelete removed v2.1.178); Codex has built-in subagents (default/worker/explorer) + spawn_agents_on_csv batch (enabled by default); Gemini main-agent-to-subagent only, no peer-to-peer
Recursion     ── blocked on Gemini (runtime-hidden); allowed elsewhere — Codex caps at `[agents] max_depth = 1` by default
Per-subagent  ── enforcement NOT hook-based — Claude `claude --agent <name>`, Codex per-agent `.toml` sandbox/tools, Gemini Policy Engine `[[rule]] subagent = "..."` in `~/.gemini/policies/*.toml`
...
Plan mode     ── Claude has LLM-callable `EnterPlanMode`/`ExitPlanMode` tools · Gemini also has LLM-callable `enter_plan_mode`/`exit_plan_mode` · Codex is user-toggled only (no LLM entry tool)
...
Built-in subs ── Claude (none — bring your own) · Codex `default`/`worker`/`explorer` · Gemini `generalist`/`cli_help`/`codebase_investigator`/`browser_agent`
Scheduling    ── Claude only — `ScheduleWakeup` + `CronCreate`/`Delete`/`List`
Cross-session ── Gemini — tiered Markdown memory via `write_file`/`replace` (`save_memory` tool removed v0.40.0); Claude/Codex use AGENTS.md / project files
