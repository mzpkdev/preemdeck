...
Codex accepts `Edit` and `Write` as matcher _aliases_ for `apply_patch` — but the `tool_name` in the payload is always
...
hook). This is a _separate_ trust gate from the project-trust mechanism that controls `.codex/config.toml` — installing
...
// gemini-extension.json
{
  "hooks": {
    "AfterTool": [{
      ...
      "hooks": [{ "type": "command", "command": "${CLAUDE_PLUGIN_ROOT}/scripts/<name>.ts" }]
    }]
  }
}
...
// gemini-extension.json
{
  "hooks": {
    "AfterTool": [{
      "matcher": "(write_.*|replace)",
      "hooks": [{ "type": "command", "command": "${extensionPath}/scripts/<name>.ts" }]
    }]
  }
}
```
...
The host parses stdout as JSON. _One stray byte_ — a debug print, a `set -x`, an rc-file welcome banner — and the parser
silently treats the hook as having no decision.

```ts
// Avoid — console.log() lands on stdout
console.log(`formatting ${path}`);
process.stdout.write(JSON.stringify({ continue: true }));

// Prefer — logs go to stderr; only the JSON (or nothing) on stdout
process.stderr.write(`formatting ${path}\n`);
process.stdout.write(JSON.stringify({ continue: true }));
```
...
const { timedOut } = await spawn(["biome", "format", "--write", path], { timeoutMs: 30_000 });
if (timedOut) process.stderr.write("format hook timed out\n");
process.exit(0);
```

Always set a `timeout` in config _and_ `timeoutMs` on the script's `spawn` calls. Belt and braces — the inner timeout
...
`PostToolUse` runs _after_ the tool succeeded. The side effect already happened. Blocking only stops the _next_ model
...
slack.post(`edited ${path}`);

// Prefer — coalesce per-turn via a marker file or per-payload hash
const seen = markerRead();
if (seen.has(payload.session_id)) process.exit(0);
markerWrite(payload.session_id);
slack.post(`edited ${path}`);
```
...
{ "type": "command", "command": "${CLAUDE_PLUGIN_ROOT}/scripts/hooks/summarize.ts" }
```
...
Claude's matcher uses _either_ an exact-string list (letters/digits/`_`/`|`) _or_ JS RegExp — and it decides based on
...
`${CLAUDE_PLUGIN_ROOT}` (Claude · Codex) and `${extensionPath}` (Gemini) are only set when the hook ships _inside_ an
...
the command path turns into `/scripts/<name>.ts`, no script found.
...
(`dirname(dirname(fileURLToPath(import.meta.url)))`) and drop the placeholder from the config.
...
try {
  ...
  process.stderr.write(`format hook: ${exc instanceof Error ? exc.message : String(exc)} — is \`biome\` installed?\n`);
}
```

A missing formatter binary warns on stderr but never blocks the edit. Document the binaries the hook expects in the
plugin's README. CI doesn't catch missing dev deps the way a `Makefile` would.
...
Stdout         ── only JSON; profile chatter + console.log() break the parser
Timeouts       ── set in config and in spawn (timeoutMs); the host won't wait forever
