...
let dir = ""
beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), "preemdeck-caveman-injhook-"))
})
afterEach(async () => {
...
describe("inject-hook", () => {
    context("extracting --event and --every", () => {
        it("pulls --event and returns operands regardless of position", () => {
            expect(extractArgs(["CAVEMAN.md", "--event", "BeforeAgent", "hosts/h.md"])).toEqual({
                event: "BeforeAgent",
                every: null,
                positionals: ["CAVEMAN.md", "hosts/h.md"]
            })
        })
        ...
        it("parses --every and keeps it out of the operands", () => {
            expect(extractArgs(["CAVEMAN.md", "--event", "UserPromptSubmit", "--every", "1"])).toEqual({
                event: "UserPromptSubmit",
                every: 1,
                positionals: ["CAVEMAN.md"]
            })
        })
    ...
    context("running the CLI", () => {
        it("emits the caveman template as additionalContext", async () => {
            const template = await writeTmp("# CAVEMAN\n\nRespond terse like smart caveman.\n")
            const { out } = await runHookCli([template], "{}")
            const ctx = JSON.parse(out).hookSpecificOutput.additionalContext
            expect(ctx).toContain("Respond terse like smart caveman.")
        })
