...
/**
 * inject-hook.ts — caveman context injector.
 *
 * Resolves a template (positional path), reads it from the plugin root,
 * substitutes the optional host-tools file's contents for `{{host_tools}}`,
 * strips, and injects via lib/hook.ts. Missing/empty files are a silent `{}`
 * no-op; a missing host-tools file substitutes empty. `--event <name>` (first
 * only) is the required host event; stdin wins.
 *
