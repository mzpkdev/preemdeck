/**
 * coerce.ts — scalar option coercers for the holo command files.
 *
 * Mirrors wire's `coerce.ts` integer coercer (kept local, not imported across the
 * plugin boundary): an arity-1 option carrying `coerce: integer` runs this only
