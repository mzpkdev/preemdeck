#!/usr/bin/env bun
/**
 * serve.ts — the FOREGROUND holo diagram server (blocking).
 *
 * Serve a graph spec file (`.json`) as a live, rendered diagram over HTTP — a
 * browser view of a graph an AGENT authors declaratively (nodes + edges; ELK
 * computes positions, React Flow renders). Resolves + validates the target file,
 * builds an inline Vite dev config (no vite.config on disk) rooted at the
 * committed `app/` template, mounts a tiny `/__holo/graph` endpoint that READS
 * the file (to seed the in-browser renderer) and WRITES a browser edit back to
 * it, and watches the file so an agent rewrite pushes a full reload to the page.
 * The on-disk JSON stays the canonical artifact the agent owns. Boots Vite in
 * dev mode, prints the greppable `holo: ready` banner, and blocks until a signal.
 *
 * Like planner, the endpoint is bidirectional: the page reads the graph (GET)
 * and POSTs edits back — write-back to the SAME file the agent owns. The watcher
 * still pushes a full reload when the file changes underneath the page, but a
 * browser's OWN write would otherwise bounce back as a reload, so the write
 * records its exact bytes and an echo predicate ({@link shouldReloadOnChange})
 * suppresses that one self-inflicted reload.
 *
 ...
 * is factored out pure too ({@link handleGraphIo}); only the real side-effects —
...
/** The repo root, allow-listed for file serving alongside the app root + the css dir. */
...
/** Extensions diagram accepts as a graph source; the browser renders the JSON spec. */
const GRAPH_EXTENSIONS = new Set([".json"])
 ...
 *
 * A hook may spawn a server then open a tab; if that tab never materializes
 * (open failed, host quit), nothing would ever connect and the process would leak
 * forever. This ceiling reaps such a never-viewed spawn. Only consulted under
 * `--kill-on-disconnect`.
 */
...
/**
 * Resolve `file` to an absolute path and assert it is an existing `.json`.
 *
 ...
 * (the runner prefixes `holo: error:`), so a missing file or a non-JSON extension
 * fails the launch as exit 1 rather than booting a server pointed at nothing.
 *
 * @param file - the positional path to the graph source; resolved against cwd.
 * @returns the absolute path to the validated `.json` file.
 * @throws {ServeError} if the path does not exist, is not a regular file, or has a non-JSON extension.
 */
export const resolveGraphPath = (file: string): string => {
    const abs = path.resolve(file)
    const ext = path.extname(abs).toLowerCase()
    if (!GRAPH_EXTENSIONS.has(ext)) {
        throw new ServeError(`${file} is not a .json graph file (got '${ext || "no extension"}').`)
    }
    let stat: fs.Stats
    try {
 ...
 *
 * The `--css` override sibling of {@link resolveGraphPath}: a pure stat + extension
...
/**
 * Build the inline Vite dev config that serves the diagram page.
 *
 ...
 * template) hosting a React Flow canvas; the graph text is NOT compiled by Vite —
 * the page fetches it from the `/__holo/graph` endpoint at runtime — so no plan
 * alias is needed here, only `react()` for the app shell. The stylesheet is
 * aliased ({@link STYLE_ALIAS} → {@link DEFAULT_CSS} or the `--css` override) so it
 * themes the page chrome. `server.fs.allow` lists the app root, the stylesheet's
 * dir, and the repo root — Vite restricts file serving to `root` by default, and
 * React Flow / ELK assets under `node_modules` (and any `--css` override) live
 * outside it, so those dirs must be allow-listed.
 *
...
export const buildViteConfig = (options: ServeOptions): InlineConfig => {
    ...
    return {
        ...
        server: {
            ...
            open: options.open,
            fs: {
                // Root-restricted by default; the renderer's node_modules assets and any
...
/** The dev-server endpoint the page uses to read the graph spec (GET). */
export const GRAPH_ENDPOINT = "/__holo/graph"

/** The outcome of {@link handleGraphIo}: an HTTP status plus an optional text body/content-type. */
export type GraphIoResult = { status: number; body?: string; contentType?: string }

/**
 * Pure request logic for {@link GRAPH_ENDPOINT}: `GET` reads the graph spec,
 * `POST` writes the request body back to it, anything else is `405`. Extracted
 * from the live middleware (which only adapts Node's req/res and the file I/O
 * onto this) so the spec drives it with fake I/O — no socket, no real file. The
 * write is the whole point: an edit made in the browser lands on the same file
 * the agent reads.
 *
 * @param method - the request method (`req.method`).
 * @param readBody - reads the full request body as text; consulted on `POST` only.
 * @param io - the graph file's read/write side-effects, injected for testability.
 * @returns the status and optional body/content-type to write onto the response.
 */
export const handleGraphIo = async (
    method: string | undefined,
    readBody: () => Promise<string>,
    io: { read: () => Promise<string>; write: (text: string) => Promise<void> }
): Promise<GraphIoResult> => {
    if (method === "GET") {
        return { status: 200, body: await io.read(), contentType: "application/json; charset=utf-8" }
    }
    if (method === "POST") {
...
/** Read a Node request stream to a single UTF-8 string (the POSTed graph text). */
...
/**
 * Pure echo predicate for the file watcher: reload the page only when the changed
 * file's content differs from the exact bytes the browser's last write recorded.
 *
 * When they match, the change IS that write echoing back through the watcher — a
 * self-inflicted reload to swallow so a browser edit doesn't bounce the page. Any
 * other change (an agent rewrite, an external editor) differs and reloads.
 *
 * @param changed - the changed file's current content.
 * @param lastWritten - the exact bytes of the last browser write, or `undefined` if none is pending.
 * @returns `true` when the page should reload, `false` when the change is our own echo.
 */
export const shouldReloadOnChange = (changed: string, lastWritten: string | undefined): boolean =>
    changed !== lastWritten
 ...
 *
 * The `/__holo/graph` endpoint serves the file's current text (GET) and persists
 * a browser edit back to it (POST); a watcher on the same file pushes a
 * `full-reload` over Vite's HMR ws whenever it changes, so an agent rewrite
 * re-renders the page — except the page's own POST, whose echo the watcher
 * suppresses ({@link shouldReloadOnChange}) so a browser edit does not bounce.
 *
 ...
 *
 * @param file - the positional path to the graph source `.json`.
 * @param options - the resolved launch knobs; see {@link ServeOptions}.
...
export const serve = async (file: string, options: ServeOptions): Promise<void> => {
    const graphPath = resolveGraphPath(file)
    // Validate the --css override up front (like the graph path) so a bad path fails
    ...
    // transformIndexHtml that rewrites <title> to the graph's basename.
    const title = escapeHtmlText(path.basename(graphPath))
    config.plugins = [
        ...(config.plugins ?? []),
        {
            name: "holo-title",
        ...
        {
            // Mount the endpoint the page reads the graph from and POSTs edits back
            // to, and watch the file so an agent rewrite pushes a full reload. GET
            // seeds the renderer with the file's current text; POST persists a browser
            // edit to the SAME file. The decision is pure (handleGraphIo); this only
            // adapts Node's req/res and the file I/O onto it.
            name: "holo-graph",
            configureServer(server: ViteDevServer): void {
                // The exact bytes of the browser's last write, held so the watcher can
                // tell that write's own change event apart from an agent's edit and
                // skip the echo reload. Reset to undefined once that echo is consumed.
                let lastWritten: string | undefined

                server.middlewares.use(GRAPH_ENDPOINT, (req, res) => {
                    void handleGraphIo(req.method, () => readRequestBody(req), {
                        read: () => fs.promises.readFile(graphPath, "utf8"),
                        write: async (text) => {
                            await fs.promises.writeFile(graphPath, text)
                            // Record the exact bytes just written so the watcher's
                            // change event for this write is recognized as our own echo.
                            lastWritten = text
                        }
                    })
                        .then((result) => {
                ...
                // The graph file lives outside the Vite root, so add it to the watcher
                // explicitly. On change, reload the page — unless the change is the
                // browser's own POST echoing back, in which case swallow it once and
                // re-arm for the next real change.
                server.watcher.add(graphPath)
                server.watcher.on("change", (changed) => {
                    if (path.resolve(changed) !== graphPath) {
                        return
                    }
                    void fs.promises
                        .readFile(graphPath, "utf8")
                        .then((content) => {
                            if (shouldReloadOnChange(content, lastWritten)) {
                        ...
                        .catch(() => {
                            // Couldn't read to compare (transient mid-write / unlink):
                            // fall back to a reload so the page reconciles via GET.
                            server.ws.send({ type: "full-reload" })
                        })
                })
            }
    ...
    if (server === undefined) {
        ...
        process.stdout.write(`holo: ready url=http://${options.host}:${options.port} graph=${graphPath}\n`)
        return
    }
    ...
    process.stdout.write(`holo: ready url=${url} graph=${graphPath}\n`)
    server.printUrls()
...
const command = defineCommand({
    name: "serve",
    description:
        "Serve a graph spec (.json) as a live, rendered diagram over HTTP — an agent authors it, the page renders it.",
    arguments: [{ name: "file", description: "path to the .json graph spec to render", required: true }],
    options: [
        {
            name: "host",
            arity: 1,
        ...
        },
        {
            ...
            arity: 0,
            description:
                "exit when the last HMR client disconnects — used by an interactive hook so each spawned server self-cleans on tab close"
        },
        {
            name: "css",
            arity: 1,
            hint: "path",
            description: "override the page stylesheet with a .css file (default: diagram's built-in style.css)"
        }
    ],
