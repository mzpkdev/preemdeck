...
```bash
"$HOME/.preemdeck/preemdeck-runtime" "$HOME/.preemdeck/src/ripperdoc/dock/idea/toolbox/in-idea.ts"        # prints "in a JetBrains IDE terminal" / "not …"
"$HOME/.preemdeck/preemdeck-runtime" "$HOME/.preemdeck/src/ripperdoc/dock/idea/toolbox/in-idea.ts" -q && echo "good to go"   # quiet (-q/--silent): gate on the exit code
```
