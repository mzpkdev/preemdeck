...
describe("launch", () => {
    context("spawning the resolved launcher", () => {
        it("default spawns the exact argv, async (no --wait, does not block)", async () => {
        ...
        it("wait:false does not append the flag or block", async () => {
            const { spawn, last } = fakeSpawn()
        ...
        it("wait:true appends --wait at the END and blocks on the child's exit", async () => {
            const { spawn, last } = fakeSpawn()
        ...
        it("returns the (completed) child handle", async () => {
    ...
    context("when the launcher cannot be resolved", () => {
        it("propagates IdeaError and never reaches spawn when resolve throws", async () => {
